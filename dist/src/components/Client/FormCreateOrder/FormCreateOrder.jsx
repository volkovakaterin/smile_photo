...
export const FormCreateOrder = memo(({ confirmFn, error }) => {
    ...
    if (!error)
        return (<form onSubmit={handleSubmit(onSubmit)} autoComplete="off" className={styles.FormCreateOrder}>
            ...
            }} render={({ field, fieldState }) => (<InputPhone field={field} fieldState={fieldState} errors={errors} setPhoneExistsError={setPhoneExistsError} phoneExistsError={phoneExistsError} type={TYPE_INPUT.CREAT}/>)}/>
                    </label>
                </div>
        ...
        return (<div>Ошибка при открытии заказа</div>);
    }
});
//# sourceMappingURL=FormCreateOrder.jsx.map