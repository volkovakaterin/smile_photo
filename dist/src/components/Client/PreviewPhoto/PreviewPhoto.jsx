...
export const PreviewPhoto = memo(({ open, handleClose, activeSlide, images, toggleSelect, checkSelectPhoto, selectPhotos, dir }) => {
    const settings = {
        dots: false,
    ...
    if (!open || activeSlide === null)
        return null;
    return (<div className={styles.PreviewPhoto}>
            <div className={styles.wrapperBox}>
                {handleClose && (<div className={styles.wrapperClose} onClick={handleClose}>
                    <Image src={CloseIcon} alt={'close'} width={20} height={20}/>
                </div>)}
                <Slider {...settings} className={styles.slider}>
                    {images.map((image, index) => (<SlidePreview key={index} image={image} selectPhotos={selectPhotos} toggleSelect={toggleSelect} index={index} checkSelectPhoto={checkSelectPhoto} dir={dir}/>))}
                </Slider>
            </div>

        </div>);
});
//# sourceMappingURL=PreviewPhoto.jsx.map